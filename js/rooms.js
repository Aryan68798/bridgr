...
function renderList() {
  ...
  list.innerHTML = rooms
    .map((r) => {
      ...
      const isCreator = isLoggedIn && r.creator === isLoggedIn;
      return `<div class="rli${selectedRoom?.id === r.id ? " selected" : ""}" data-id="${r.id}">
      <div class="rli-dot ${dotClass}"></div>
      <div class="rli-body">
        <div class="rli-name">${r.name} ${isCreator ? '<span class="your-badge">YOU</span>' : ''}</div>
        <div class="rli-meta">${r.building} · Floor ${r.floor}</div>
        ${r.session ? `<div class="rli-session">${isCreator ? '<span style="color:var(--orange);font-weight:800;">Your Session:</span> ' : ''}"${r.session}"</div>` : ""}
